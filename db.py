        ...
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    # 1. Снятие всех старых NOT NULL ограничений
                    cur.execute("""
                        ...
                        -- Безопасная очистка ограничений со старых полей
                        DO $$ 
                        DECLARE 
                            col RECORD;
                        BEGIN 
                            FOR col IN 
                                ...
                                WHERE table_name = 'accounts' AND column_name NOT IN ('id')
                            LOOP 
                                EXECUTE format('ALTER TABLE accounts ALTER COLUMN %I DROP NOT NULL;', col.column_name);
                            END LOOP;
                        END $$;
                    """)
                    ...
                    """)
                    conn.commit()
            logger.info("✅ БД и ограничения колонок успешно подготовлены.")
